...
const LeftSidebar = () => {
    ...
    const logoutHandler = async () => {
        try {
            const res = await axios.get('https://bondify-4u50.onrender.com/api/v1/user/logout', { withCredentials: true });
            if (res.data.success) {
    ...
    return (
        <div className='fixed top-0 z-10 left-0 px-4 border-r border-gray-300 w-[16%] h-screen bg-white'>
            ...
            <h1 className="my-8 pl-3 font-bold text-4xl">
  <span className="text-[rgb(125,198,247)] font-extrabold">Bond</span>
  <span className="font-[cursive]">ify</span>
</h1>
                <div className="w-full h-[2px] bg-[rgb(125,198,247)] my-4"></div>
                <div>
